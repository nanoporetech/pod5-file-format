...
import mkr_format
import mkr_format.signal_tools
...
def gen_test_read(seed) -> ReadData:
    ...
    return ReadData(
        ...
        mkr_format.CalibrationData(get_random_float(), get_random_float()),
        get_random_int(0, 100000),
        get_random_int(0, 10000000),
        get_random_float(),
        mkr_format.EndReasonData(
            mkr_format.EndReason(get_random_int(0, 5)), get_random_int(0, 1)
        ),
        mkr_format.RunInfoData(
...
def run_writer_test(f):
    ...
    f.add_read(
        ...
        f.find_end_reason(**test_read.end_reason._asdict())[0],
        f.find_run_info(**test_read.run_info._asdict())[0],
        mkr_format.signal_tools.vbz_compress_signal(test_read.signal),
        test_read.signal.shape[0],
    ...
    f.add_reads(
        ...
        [mkr_format.signal_tools.vbz_compress_signal(r.signal) for r in test_reads],
        numpy.array([len(r.signal) for r in test_reads], dtype=numpy.uint64),
        pre_compressed_signal=True,
    )
...
def test_pyarrow_combined():
    with tempfile.TemporaryDirectory() as td:
        ...
        with mkr_format.open_combined_file(path, use_c_api=False) as r:
            run_reader_test(r)
...
def test_pyarrow_split():
    with tempfile.TemporaryDirectory() as td:
        ...
        with mkr_format.open_split_file(signal, reads, use_c_api=False) as r:
            run_reader_test(r)
...
def test_pyarrow_split_one_name():
    with tempfile.TemporaryDirectory() as td:
        ...
        with mkr_format.open_split_file(p, use_c_api=False) as r:
            run_reader_test(r)
