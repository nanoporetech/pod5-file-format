...
- PDZ signal compression (`minknow.pdz`) as a user-selectable alternative to VBZ.
  Selectable via the C API (`PDZ_SIGNAL_COMPRESSION`), the pybind `SignalType.PdzSignal`,
  and `pod5.Writer(signal_compression_type=pod5.SignalType.PdzSignal)`. Readers detect
  and decode PDZ files, and `pod5 inspect` reports PDZ compression.
