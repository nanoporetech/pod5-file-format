...
public:
    CachedBatchSignalData(std::uint32_t batch_index, std::size_t entry_count)
            : m_batch_index(batch_index), m_sample_counts(entry_count), m_samples(entry_count) {}
...
public:
    SignalCacheWorkPackage(std::uint32_t batch_index,
...
public:
    ...
    AsyncSignalLoader(std::shared_ptr<pod5::FileReader> const& reader,
...
private:
    ...
    void run_worker();
    void do_work(std::shared_ptr<SignalCacheWorkPackage> const& batch,
