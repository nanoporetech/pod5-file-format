...
RunInfoTableReader::RunInfoTableReader(RunInfoTableReader&& other)
        : TableReader(std::move(other)), m_field_locations(std::move(other.m_field_locations)) {}
...
Result<std::shared_ptr<RunInfoData const>> RunInfoTableReader::find_run_info(
        ...
        for (std::size_t j = 0; j < batch.num_rows(); ++j) {
            if (acq_id->Value(j) == acquisition_id) {
                ...
                run_info = std::make_shared<RunInfoData>(
                        ...
                        columns.acquisition_start_time->Value(j), columns.adc_max->Value(j),
                        columns.adc_min->Value(j), value_for_map(columns.context_tags, j),
                        ...
                        columns.sample_id->Value(j).to_string(), columns.sample_rate->Value(j),
    ...
    if (!run_info) {
        return arrow::Status::Invalid("Failed to find acquisition id '", acquisition_id,
                                      "' in run info table");
    }
...
Result<RunInfoTableReader> make_run_info_table_reader(
    ...
    ARROW_ASSIGN_OR_RAISE(auto field_locations,
                          read_run_info_table_schema(read_metadata, reader->schema()));

    return RunInfoTableReader({input}, std::move(reader), field_locations, std::move(read_metadata),
                              pool);
}
