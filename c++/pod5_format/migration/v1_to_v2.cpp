...
arrow::Result<std::size_t> get_num_samples(
    ...
    for (std::int64_t index = 0; index < signal_col->value_length(row_idx); ++index) {
        ...
        if (batch_idx >= signal_batches.size()) {
            return arrow::Status::Invalid("Invalid signal row ", abs_row,
                                          ", cannot find signal batch ", batch_idx);
        }
        ...
        if (batch_row >= (std::size_t)samples_column->length()) {
            return arrow::Status::Invalid("Invalid signal batch row ", batch_row, ", length is ",
                                          samples_column->length());
        }
...
arrow::Result<MigrationResult> migrate_v1_to_v2(MigrationResult&& v1_input,
...

    {
        ARROW_ASSIGN_OR_RAISE(auto v1_reader,
                              open_record_batch_reader(pool, v1_input.footer().reads_table));
        ARROW_ASSIGN_OR_RAISE(auto v1_signal_reader,
                              open_record_batch_reader(pool, v1_input.footer().signal_table));
        ...
        for (std::size_t batch_idx = 0;
             batch_idx < (std::size_t)v1_signal_reader.reader->num_record_batches(); ++batch_idx) {
            ARROW_ASSIGN_OR_RAISE(signal_batches[batch_idx],
                                  v1_signal_reader.reader->ReadRecordBatch(batch_idx));
        }
        ...
        for (std::int64_t batch_idx = 0; batch_idx < v1_reader.reader->num_record_batches();
            ...
            for (std::int64_t row = 0; row < num_rows; ++row) {
                ARROW_ASSIGN_OR_RAISE(auto num_samples,
                                      get_num_samples(signal_column, row, signal_batches));
