    ...
    arrow::MemoryPool * pool)
{
    ...
    // Make a writer using the V6 schema
    ARROW_ASSIGN_OR_RAISE(
        auto new_metadata, update_metadata(v5_reader.metadata, kPod5VersionReadTableV6));
    auto const field_locations = std::make_shared<ReadTableSchemaDescription>();
    auto v6_schema = field_locations->make_writer_schema(new_metadata);
    ARROW_ASSIGN_OR_RAISE(
        auto v6_writer,
