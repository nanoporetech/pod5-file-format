...
arrow::Result<StructRow> get_dict_struct(std::shared_ptr<arrow::RecordBatch> const& batch,
                                         std::size_t row,
...
arrow::Status append_struct_row(StructRow const& struct_row,
                                char const* field_name,
                                Builder& builder) {
...
arrow::Status append_struct_row_to_dict(StructRow const& struct_row,
                                        char const* field_name,
...
arrow::Result<MigrationResult> migrate_v2_to_v3(MigrationResult&& v2_input,
...

    {
...

        {
            ...
            for (std::int64_t batch_idx = 0; batch_idx < v2_reader.reader->num_record_batches();
                ...
                for (auto const& col_name : columns_to_copy) {
                    ARROW_RETURN_NOT_OK(copy_column(v2_reader.schema, v2_columns, col_name.data(),
                                                    v3_reads_schema, v3_columns));
                }
                ...
                for (std::int64_t row = 0; row < num_rows; ++row) {
                    ...
                    ARROW_ASSIGN_OR_RAISE(auto end_reason_data,
                                          get_dict_struct(v2_batch, row, "end_reason"));
                    ARROW_RETURN_NOT_OK(
                    ...
                    ARROW_ASSIGN_OR_RAISE(auto run_info_data,
                                          get_dict_struct(v2_batch, row, "run_info"));
                    ARROW_RETURN_NOT_OK(
                            append_struct_row_to_dict(run_info_data, "acquisition_id", run_info));
                }
                ARROW_RETURN_NOT_OK(set_column(v3_reads_schema, v3_columns, "calibration_offset",
                                               calibration_offset.Finish()));
                ARROW_RETURN_NOT_OK(set_column(v3_reads_schema, v3_columns, "calibration_scale",
                                               calibration_scale.Finish()));
                ARROW_RETURN_NOT_OK(
                ...
                ARROW_RETURN_NOT_OK(set_column(v3_reads_schema, v3_columns, "end_reason_forced",
                                               end_reason_forced.Finish()));
                ARROW_RETURN_NOT_OK(
        ...
        }
        {
            ...
            ARROW_RETURN_NOT_OK(v3_run_info_writer.write_batch(run_info_items->length(),
                                                               std::move(v3_columns)));
