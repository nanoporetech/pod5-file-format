...
arrow::Result<MigrationResult> migrate_v0_to_v1(MigrationResult&& v0_input,
...

    {
        ARROW_ASSIGN_OR_RAISE(auto v0_reader,
                              open_record_batch_reader(pool, v0_input.footer().reads_table));

        auto v1_new_schama =
                arrow::schema({arrow::field("num_minknow_events", arrow::uint64()),
        ...
        for (std::int64_t batch_idx = 0; batch_idx < v0_reader.reader->num_record_batches();
            ...
            ARROW_RETURN_NOT_OK(
                    set_column(v1_schema, columns, "num_minknow_events",
                               make_filled_array<arrow::UInt64Builder>(pool, num_rows, 0)));
            ARROW_RETURN_NOT_OK(
                    set_column(v1_schema, columns, "tracked_scaling_scale",
                               make_filled_array<arrow::FloatBuilder>(
                                       pool, num_rows, std::numeric_limits<float>::quiet_NaN())));
            ARROW_RETURN_NOT_OK(
                    set_column(v1_schema, columns, "tracked_scaling_shift",
                               make_filled_array<arrow::FloatBuilder>(
                                       pool, num_rows, std::numeric_limits<float>::quiet_NaN())));
            ARROW_RETURN_NOT_OK(
                    set_column(v1_schema, columns, "predicted_scaling_scale",
                               make_filled_array<arrow::FloatBuilder>(
                                       pool, num_rows, std::numeric_limits<float>::quiet_NaN())));
            ARROW_RETURN_NOT_OK(
                    set_column(v1_schema, columns, "predicted_scaling_shift",
                               make_filled_array<arrow::FloatBuilder>(
                                       pool, num_rows, std::numeric_limits<float>::quiet_NaN())));
            ARROW_RETURN_NOT_OK(
                    set_column(v1_schema, columns, "num_reads_since_mux_change",
                               make_filled_array<arrow::UInt32Builder>(pool, num_rows, 0)));
            ARROW_RETURN_NOT_OK(
                    set_column(v1_schema, columns, "time_since_mux_change",
