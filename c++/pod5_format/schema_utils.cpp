...
Status SchemaDescriptionBase::read_schema(std::shared_ptr<SchemaDescriptionBase> dest_schema,
    ...
    for (auto& field : dest_schema->fields()) {
        ...
        if (datatype->id() == arrow::Type::DICTIONARY) {
            ...
            if (dict_type->value_type()->id() == arrow::Type::STRUCT) {
                ...
                ARROW_ASSIGN_OR_RAISE(field_index, find_dict_field(schema, field->name().c_str(),
