...
#define MKR_PYTHON_RETURN_NOT_OK(result)                      \
    if (!result.ok()) {                                       \
        throw std::runtime_error(result.status().ToString()); \
    }

#define MKR_PYTHON_ASSIGN_OR_RAISE_IMPL(result_name, lhs, rexpr)   \
...
#define MKR_PYTHON_ASSIGN_OR_RAISE(lhs, rexpr)                                                     \
    MKR_PYTHON_ASSIGN_OR_RAISE_IMPL(ARROW_ASSIGN_OR_RAISE_NAME(_error_or_value, __COUNTER__), lhs, \
                                    rexpr);
...
void throw_on_error(mkr::Status const& s) {
    if (!s.ok()) {
...
template <typename T>
T throw_on_error(mkr::Result<T> const& s) {
    if (!s.ok()) {
        throw std::runtime_error(s.status().ToString());
    }
    return *s;
}
std::shared_ptr<mkr::FileWriter> create_combined_file(char const* path,
                                                      std::string const& writer_name,
                                                      mkr::FileWriterOptions const* options) {
    mkr::FileWriterOptions dummy;
    MKR_PYTHON_ASSIGN_OR_RAISE(
            auto writer, mkr::create_combined_file_writer(
                                 path, writer_name, options ? *options : mkr::FileWriterOptions{}));
    return writer;
}

std::shared_ptr<mkr::FileWriter> create_split_file(char const* signal_path,
                                                   char const* reads_path,
                                                   std::string const& writer_name,
                                                   mkr::FileWriterOptions const* options) {
    mkr::FileWriterOptions dummy;
    MKR_PYTHON_ASSIGN_OR_RAISE(auto writer, mkr::create_split_file_writer(
                                                    signal_path, reads_path, writer_name,
                                                    options ? *options : mkr::FileWriterOptions{}));
    return writer;
}

struct MkrFileReaderPtr {
    MkrFileReader_t* reader = nullptr;

    MkrFileReaderPtr(MkrFileReader_t* reader_) : reader(reader_) {}

    EmbeddedFileData get_combined_file_read_table_location() const {
        EmbeddedFileData result;
        auto error = mkr_get_combined_file_read_table_location(reader, &result);
        if (error != MKR_OK) {
            throw std::runtime_error(mkr_get_error_string());
        }
        return result;
    }
    ...
    EmbeddedFileData get_combined_file_signal_table_location() const {
        EmbeddedFileData result;
        auto error = mkr_get_combined_file_signal_table_location(reader, &result);
        if (error != MKR_OK) {
            throw std::runtime_error(mkr_get_error_string());
        }
        return result;
    ...
    void close() {
        auto error = mkr_close_and_free_reader(reader);
        if (error != MKR_OK) {
            throw std::runtime_error(mkr_get_error_string());
        }
        reader = nullptr;
    ...
    std::size_t plan_traversal(
        ...
        auto error = mkr_plan_traversal(
                reader, read_id_data.data(), read_id_data.shape(0),
                (mkr_traversal_sort_type_t)sort_order,
...
MkrFileReaderPtr open_combined_file(char const* filename) {
    auto reader = mkr_open_combined_file(filename);
    if (!reader) {
        throw std::runtime_error(mkr_get_error_string());
    }
    return MkrFileReaderPtr(reader);
}

MkrFileReaderPtr open_split_file(char const* signal_filename, char const* reads_filename) {
    auto reader = mkr_open_split_file(signal_filename, reads_filename);
    if (!reader) {
        throw std::runtime_error(mkr_get_error_string());
    }
    return MkrFileReaderPtr(reader);
...
mkr::RunInfoDictionaryIndex FileWriter_add_run_info(
        mkr::FileWriter& w,
...
void FileWriter_add_reads(
    ...
    for (std::size_t i = 0; i < count; ++i, ++signal_it) {
        ...
        throw_on_error(w.add_complete_read(
                mkr::ReadData{read_ids[i], *pores.data(i), *calibrations.data(i),
...
void FileWriter_add_reads_pre_compressed(
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        throw_on_error(w.add_complete_read(
                mkr::ReadData{read_id, *pores.data(i), *calibrations.data(i), *read_numbers.data(i),
                              *start_samples.data(i), *median_befores.data(i), *end_reasons.data(i),
                              *run_infos.data(i)},
                signal_rows));
    }
}
...
void decompress_signal_wrapper(
    ...
    throw_on_error(mkr::decompress_signal(
...
std::size_t compress_signal_wrapper(
    ...
    auto size = throw_on_error(mkr::compress_signal(
...
PYBIND11_MODULE(mkr_format_pybind, m) {
    using namespace mkr;
    mkr_init();

    m.doc() = "MKR Format Raw Bindings";
    ...
    py::class_<FileWriter, std::shared_ptr<FileWriter>>(m, "FileWriter")
            .def("close", [](mkr::FileWriter& w) { throw_on_error(w.close()); })
            .def("add_pore",
                 [](mkr::FileWriter& w, std::uint16_t channel, std::uint8_t well,
                    std::string pore_type) {
                     return throw_on_error(
                             w.add_pore(mkr::PoreData{channel, well, std::move(pore_type)}));
                 })
            .def("add_end_reason",
                 [](mkr::FileWriter& w, int name, bool forced) {
                     return throw_on_error(w.add_end_reason(
                             mkr::EndReasonData{(mkr::EndReasonData::ReadEndReason)name, forced}));
                 })
            .def("add_calibration",
                 [](mkr::FileWriter& w, float offset, float scale) {
                     return throw_on_error(w.add_calibration(mkr::CalibrationData{offset, scale}));
                 })
    ...
    py::class_<MkrFileReaderPtr>(m, "MkrFileReader")
            .def("get_combined_file_read_table_location",
                 &MkrFileReaderPtr::get_combined_file_read_table_location)
            .def("get_combined_file_signal_table_location",
                 &MkrFileReaderPtr::get_combined_file_signal_table_location)
            .def("plan_traversal", &MkrFileReaderPtr::plan_traversal)
            .def("close", &MkrFileReaderPtr::close);

    // Errors API
    m.def("get_error_string", &mkr_get_error_string, "Get the most recent error as a string");

    // Creating files
    m.def("create_combined_file", &create_combined_file, "Create a combined MKR file for writing",
          py::arg("filename"), py::arg("writer_name"), py::arg("options") = nullptr);
    m.def("create_split_file", &create_split_file, "Create a split MKR file for writing",
          py::arg("signal_filename"), py::arg("reads_filename"), py::arg("writer_name"),
          py::arg("options") = nullptr);

    // Opening files
    m.def("open_combined_file", &open_combined_file, "Open a combined MKR file for reading");
    m.def("open_split_file", &open_split_file, "Open a split MKR file for reading");
