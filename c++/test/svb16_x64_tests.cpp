...
void test_sse_encode_scalar_decode() {
    ...
    std::uniform_int_distribution<Int16T> dist{std::numeric_limits<Int16T>::min(),
                                               std::numeric_limits<Int16T>::max()};
    ...
    auto const encoded_count = svb16::encode_sse<Int16T, UseDelta, UseZigzag>(
                                       ...
                                       encoded.data() + svb16_key_length(data.size()), DATA_COUNT) -
                               encoded.data();
    ...
    auto const scalar_encoded_count =
            svb16::encode_scalar<Int16T, UseDelta, UseZigzag>(
                    data.data(), encoded_scalar.data(),
                    encoded_scalar.data() + svb16_key_length(data.size()), DATA_COUNT) -
            encoded_scalar.data();
